...
class _FlashcardState extends State<Flashcard> {
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedFlipWidget(
      ...
      flipDirection: MediaQuery.of(context).orientation == Orientation.portrait ? FlipDirection.horizontal : FlipDirection.vertical,
    );
  }
}
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Row(
              children: [
                ConstrainedBox(
                  constraints: BoxConstraints(maxHeight: MediaQuery.of(context).size.height / 3.75, maxWidth: MediaQuery.of(context).size.width / 1.6),
                  child: TextField(
                    maxLength: 150,
            ...
            if(frontImage != null || frontImageData != null) Padding(
              padding: const EdgeInsets.all(8.0),
              child: Row(
            ...
            if(backImage != null || backImageData != null) Padding(
              padding: const EdgeInsets.all(8.0),
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
            ...
            Row(
              children: [
                ...
                TextButton(
                  onPressed: () async {
                    if(kIsWeb) {
                      await createFlashcard(front.text, back.text, widget.collectionPath, frontImageData, backImageData);
                    } else {
                      await createFlashcard(front.text, back.text, widget.collectionPath, frontImage, backImage);
                    }
                    if(context.mounted) {
                      Navigator.pop(context);
                    }
...
Future<void> createFlashcard(String front, String back, String collectionPath, dynamic frontImage, dynamic backImage) async {
  String? frontImageLink;
  String? backImageLink;
  if(frontImage.runtimeType.toString() == '_File') {
    final imageRef = FirebaseStorage.instance.ref().child('$collectionPath/${DateTime.now()}');
  ...
  QuerySnapshot<Map<String, dynamic>> snapshot = await db.collection(collectionPath).get();
  db.collection(collectionPath).add({
    ...
    'Front Image' : frontImageLink,
    'Back Image' : backImageLink,
  });
}
...
class FlashcardInfo extends StatelessWidget {
  ...
  const FlashcardInfo.reorderable({
    super.key,
    required this.front,
    required this.back,
    required this.collectionPath,
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        ...
        mainAxisAlignment: reorder ? MainAxisAlignment.start : MainAxisAlignment.spaceBetween,
        children: [
          if(reorder) const Padding(
            padding: EdgeInsets.only(right: 15),
            child: Icon(Icons.reorder_rounded),
          ),
          Column(
            children: [
              if(frontImage != null) ClipRRect(
          ...
          Flexible(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(back) // TODO: add text style
              ],
            ),
          ),
          if (!reorder) PopupMenuButton(
            iconSize: 20,
            ...
            itemBuilder: (context) => [
              PopupMenuItem(
                onTap: () => showDialog<String>(
                    ...
                    builder: (BuildContext context) => Dialog(
                      child: EditFlashcard(collectionPath: collectionPath, id: id, front: front, back: back, frontImageLink: frontImage,),
                    )
                ),
              ...
              PopupMenuItem(
                onTap: () async {
                  if(frontImage != null){
                    final Reference imageRef = FirebaseStorage.instance.refFromURL(frontImage!);
                    await imageRef.delete();
                  }
                  if(backImage != null){
                    final Reference imageRef = FirebaseStorage.instance.refFromURL(backImage!);
                    await imageRef.delete();
                  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            if(frontStoredImage != null) Padding(
              padding: const EdgeInsets.all(8.0),
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
            ...
            if(backStoredImage != null) Padding(
              padding: const EdgeInsets.all(8.0),
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
            ...
            Row(
              children: [
                ...
                TextButton(
                  onPressed: () async {
                    if(frontStoredImage == null && widget.frontImageLink != null) {
                      final Reference imageRef = FirebaseStorage.instance.refFromURL(widget.frontImageLink!);
                      await imageRef.delete();
                    }
                    if(newFrontImage != null) { // No image -> New image,
                      final Reference imageRef = FirebaseStorage.instance.ref().child('${widget.collectionPath}/${DateTime.now()}'); // Cloud Location
                      await imageRef.putFile(newFrontImage!); // Upload image to cloud
                      frontStoredImage = await imageRef.getDownloadURL(); // Get URL to ref image
                    }
                    if(backStoredImage == null && widget.backImageLink != null) {
                      final Reference imageRef = FirebaseStorage.instance.refFromURL(widget.backImageLink!);
                      await imageRef.delete();
                    }
                    if(newBackImage != null) { // No image -> New image,
                      final Reference imageRef = FirebaseStorage.instance.ref().child('${widget.collectionPath}/${DateTime.now()}'); // Cloud Location
                      await imageRef.putFile(newFrontImage!); // Upload image to cloud
                      backStoredImage = await imageRef.getDownloadURL(); // Get URL to ref image
                    }
